...
| `vector_jepa_frontier_confirmatory` | 900 | 仅机器校验hash/隔离；禁止评测或选择 | 不可以 |

full-900包含尺寸9、11、13、15、17、19、21、23、25，每个尺寸100任务；seen为
9-21共700任务，OOD为23/25共200任务。四个manifest必须在`(maze_size,
topology_seed)`、layout hash和task hash三层两两无交集；各自内部也不得重复。
...
除B0必须恰好使用768次外，新搜索器可以因提前终止少用预算；因此`1x`表示共同
上限，不表示每个算法的实际调用数相等。实际使用量必须逐decision记录并最终报告。
...
- Vector-DTS vs 同预算、同value/scorer的uniform-expansion MCTS；另保留
  search-disabled direct-DTS作为不参与晋级门槛的描述性诊断；
- Bidirectional vs 共享verifier/join语义的forward-only Best-first；
- Denoising-iCEM vs uniform-iCEM。

Vector-DTS正式对照只改变learned policy prior；Bidirectional和Denoising比较的是
预先固定的完整算法系统。所有正式对照共享backbone、训练split、1x上限和评测任务，
但不得把“共同上限”误写成实际compute完全相等。

对Vector-DTS learned/uniform/direct三者，以及Bidirectional/forward-only两者，脚本
还会在比较前逐张量验证共享head参数、训练损失（仅排除wall time）、校准指标、源
backbone和manifest哈希完全一致。该检查失败时不生成shortlist；因此这些对照中的
搜索差异不会混入一次独立head训练造成的参数漂移。
...
只冻结一个胜者。Q4为胜者、B0和胜者的预注册匹配控制补齐backbones42-51；有head的
方法再补第二planner seed。
...
5. Q2候选相对预注册匹配控制的SR增量至少0.02，且Bonferroni同时区间下界大于0。

同时区间的比较家族在运行前固定为48项：12个可晋级候选乘2个action protocols，
再乘2类对照（B0系统效应、匹配控制机制效应）。family-wise alpha为0.05，因此每项
bootstrap区间使用alpha=`0.05/48`。seed42筛选利用success差值只可能为-1/0/1的
性质，按maze size对经验bootstrap分布做多项式卷积并精确枚举，不使用有限次Monte
Carlo估计极端尾部分位数。即使Q1不以机制效应作为门槛，也保留在同一比较家族中，
不根据结果缩小分母。该区间只刻画固定backbone seed42上的task-panel不确定性，
不能解释为跨训练seed的模型不确定性。
...
compute为二级结果。最终JSON和Markdown都必须报告10-backbone mean/SD、逐尺寸
SR和每decision compute。Q4报告的是经过选择后的描述性、逐点95%区间，不是确认性
