...
| Q2B 激进替代 | Vector-DTS、Bidirectional、Denoising-iCEM | uniform-expansion DTS、forward-only、uniform-iCEM；Direct-DTS仅诊断 |
| Q2C 反例学习 | Hard-negative ranker | random-negative ranker |

一共12个可晋级方法族、19个可执行方法（含B0、bridge和控制）。所有Q2A方法独立
运行，不累计装配。Q2B和Q2C也不与Q2A组合。
