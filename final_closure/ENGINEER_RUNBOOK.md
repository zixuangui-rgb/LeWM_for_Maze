...
The runner validates an existing audit, checkpoint, or result before skipping it. A stale or corrupt resume artifact fails immediately instead of surviving until final summarization. It never silently overwrites formal output.
...
Immediately verify the immutable tree:

```bash
python -m final_closure.verify_closure
```

The expected paper output is nine CSV tables, five PNG figures, one Markdown report, `summary.json`, and the closure gate. The verifier re-hashes all of them plus both final/source configs and locks, manifests, audit, checkpoints, and row-level source results.
...
python -m final_closure.run_plan \
  ...
  --seeds 42 \
  --action-selections unmasked \
  --rerun-execution-failures \
  --rerun-reason manifest_checkpoint_or_code_hash_mismatch \
  --device cuda
```

Replacement requires exactly one explicit stage, baseline, seed, and evaluation action protocol. The new output records the allowed reason and SHA256 of the superseded file. Broad `full` replacement is rejected. Never use replacement because a score is low, unexpectedly high, or inconvenient.
It cannot be used after `CLOSURE_COMPLETE.json` exists. A completed study is immutable.

If interruption leaves no atomic output, run the ordinary narrow command without replacement flags. `--overwrite` is only for an existing objective failure, not a missing file.
...
| Existing corrupt formal output | Replace only the exact output with an allowed reason; retain the recorded old SHA256 |
...
A closure gate cannot be generated with `--allow-dirty-worktree`, including for testing.
